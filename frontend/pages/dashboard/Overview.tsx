  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group text-left">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center py-4 border-b border-slate-800">
                ...
                <span className="text-white font-bold uppercase">{user?.authLevel} | {user?.department}</span>
              </div>
          ...
          <div className="space-y-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="flex gap-4 items-center">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-800">Truy cập tài liệu: SECURE_VAULT_{i}.pdf</p>
                  <p className="text-[10px] text-slate-400 uppercase font-black tracking-tighter">14:20 PM • IP: 10.0.0.{Math.floor(Math.random() * 255)}</p>
                </div>
              </div>
            ))}
          </div>
